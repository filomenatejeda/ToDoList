...
# Se importa las librerías necesarias para el desarrollo de la aplicación
...
def toggle_subtarea(tarea_idx, subtarea_idx):
    ...
    # Se redirige a la página de tareas después de alternar la completación de la subtarea
    return redirect(url_for('ver_tareas'))

# Se define la ruta para alternar la completación de una subtarea a través de la búsqueda
@app.route("/toggle_subtarea_buscar/<int:tarea_idx>/<int:subtarea_idx>", methods=["POST"])
# Se define la función para alternar la completación de una subtarea a través de la búsqueda
def toggle_subtarea_buscar(tarea_idx, subtarea_idx):
    t = gestor.lista_tareas[tarea_idx]
    subt = t.subtareas[subtarea_idx]
    subt["completada"] = not subt["completada"]
    gestor.guardar_en_archivo()
    # Se redirige a la página de búsqueda después de alternar la completación de la subtarea
    return redirect(url_for('buscar_tareas'))
